...
**Branch:** `fix/issue-70-json-parsing` (merged)
**PR:** https://github.com/vitorsilva/saberloop/pull/71
...
- [x] Deploy to production - January 3, 2026
...
**Solution:** Filter chain-of-thought and increase token limit.

### Fix 3: Chain-of-Thought Filtering (2026-01-03)

**Problem:** Reasoning models use tokens for internal "chain-of-thought" thinking before producing the actual answer. With `maxTokens: 200`, the model ran out of tokens during reasoning and showed raw thinking instead of an answer.

**Solution:**
1. Increased `maxTokens` from 200 to 500 for wrong answer explanation
2. Filter chain-of-thought - Don't use reasoning field if it starts with "Okay", "Let me", "First", etc.
3. Only use reasoning as fallback if it looks like an actual answer

**Commits:**
- `5f35908` - fix(api): filter chain-of-thought from reasoning models (#70)
...
| File | Changes |
|------|---------|
| `src/api/api.real.js` | JSON extraction improvements, increased maxTokens |
| `src/api/api.real.test.js` | 7 new tests for JSON parsing edge cases |
| `src/api/openrouter-client.js` | Chain-of-thought filtering logic |
| `src/api/openrouter-client.test.js` | 3 new tests for reasoning model handling |
