...
**Important:** This fix only takes effect after deploying the telemetry component to the VPS.

### Deployment Steps

1. Ensure your `.env` has FTP credentials configured:
   ```
   FTP_HOST=your-host
   FTP_USER=your-user
...
2. Deploy the telemetry endpoint:
   ```bash
   npm run deploy:telemetry
   ```

3. Verify the fix is deployed by checking a new telemetry log entry on the VPS:
   - Connect to VPS via FTP/SSH
   - Check `/telemetry/logs/telemetry-YYYY-MM-DD.jsonl`
   - Confirm `_server` object no longer contains `clientIp` field
