...
- 🛡️ **Prevent regression** - CI catches new dead code before merge
...
### Phase 1: Initial Setup & Discovery (Session 1)
...
### Phase 2: Triage & Ignore List (Session 1-2)
...
|----------|--------|---------|
| **Truly unused** | Remove in Phase 3 | Old helper function |
...
### Phase 4: CI Integration - Warning Mode (Session 2-3)
...
### Phase 5: CI Integration - Blocking Mode (Session 3)
...
Add to `CLAUDE.md` or create `CONTRIBUTING.md`:
...
| Week | Action | CI Behavior |
|------|--------|-------------|
| 1 | Install, discover, triage | Not in CI |
| 1-2 | Cleanup obvious dead code | Not in CI |
| 2-3 | Add to CI as warning | Reports but doesn't fail |
| 4+ | Promote to blocking | Fails build on issues |
...
### False Positives: Netlify Functions

Netlify Functions are entry points but may appear unused:

**Solution:** Add them to `entry` array in config:
```json
{
  "entry": [
    "src/main.js",
...
**Initial effort:** 2-3 sessions (discovery + cleanup + CI)

**Ongoing effort:** Minimal
- Knip runs automatically in CI
- Only action needed when it flags new issues
- Occasional ignore list updates

**Dependencies added:** 1 (`knip` as devDependency)
...
- Parking Lot: [Architecture Testing](./ARCH_TESTING_JS_ARCHUNIT.md) (related static analysis)
...
**Status:** Ready to Implement
**Location:** `docs/learning/parking_lot/DEAD_CODE_DETECTION.md`
